...
void UnidadControl::obtener_operacion() {
	...
	if (opcodeIn.read() >= ADD && opcodeIn.read() <= SRA) {
		ALUSrcOut.write(false);         // el 2do operando de la ALU será el 2do registro leido del AR
	...
	// instruccion de tipo L
		...
		RegWriteOut.write(true);        // se escribe enel registro destino el valor presente en write data
		MemWriteOut.write(false);       // no se permite la escritura en DataMemory
